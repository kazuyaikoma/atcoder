...
use bitonic_sorter::utils::{new_u32_vec, is_sorted};
...
fn main() {
  if let Some(n) = env::args().nth(1) {
    let bits = u32::from_str(&n).expect("error parcing argument");
    run_sorts(bits);
  } else {
    eprintln!(
...
fn run_sorts(bits: u32) {
  ...
  // print processor's physical cores' size and logical cores' size
  println!(
